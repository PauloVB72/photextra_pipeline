...
DEFAULT_TARGETS_CSV = ("/home/polo/Escritorio/PHD/CHANCES/"
                       "MKW8_cluster_members_070126.csv")
DEFAULT_OUT = "/home/polo/Escritorio/PHD/CHANCES/MKW8_full_run"
...
STAGES = ("photometry", "spectroscopy", "both")
# fit-stage workers (16-core box: leave headroom); spectroscopy also hits
# SPARCL/Data Lab over the network, so slightly lower there.
STAGE_WORKERS = {"photometry": 10, "spectroscopy": 8, "both": 10}
PREFETCH_WORKERS = 3
...
def _run_one(stage, tgt, out_dir):
    ...
    try:
        ...
        pipe = Pipeline(config=cfg, use_xdebpair=True, mode=stage)
...
def main():
    ...
    master = build_master_table(summary_df, args.out,
                                os.path.join(args.out, "MKW8_master_table"))
    try:
        build_pdf_report(summary_df, master,
                         os.path.join(args.out, "MKW8_run_report.pdf"))
    except Exception as exc:
